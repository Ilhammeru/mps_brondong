...
@section('content')
    ...
    <div class="row">
        <div class="col">
            <div class="card card-flush">
                <div class="card-body">
                    <form action="{{ route('trainings.update', $training->id) }}" method="POST" id="formTraining" enctype="multipart/form-data">
                        <div class="form-group row mb-5">
                            ...
                            <div class="col-md-8">
                                <input type="text" name="name" class="form-control" id="name" value="{{ $training->name }}" placeholder="Training Kesehatan & HIV">
                            </div>
                        </div>
                        <div class="form-group row mb-5">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-8">
                                <select id="pic" multiple="multiple" class="form-select form-control" name="pic[]">
                                    ...
                                    @foreach ($employee as $item)
                                        <option {{ $item->selected ? 'selected' : '' }} value="{{ $item->id }}">
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row mb-5">
                            ...
                            <div class="col-md-8">
                                <div class="input-group mb-3">
                                    <input type="date" class="form-control" value="{{ date('Y-m-d', strtotime($training->training_date)) }}" name="training_date" id="trainingDate">
                        ...
                        <div class="form-group row mb-5">
                            ...
                            <div class="col-md-8">
                                ...
                                @else
                                <textarea name="description" placeholder="{!! $description !!}" id="description" cols="3" rows="5" class="form-control">{!! $training->description !!}</textarea>
                                @endif
                            </div>
                        </div>
                        <div class="form-group row mb-5">
                            ...
                            <div class="col-md-1"></div>
                            <div class="col-md-8">
                                <input type="text" placeholder="Aula Merdeka" class="form-control" name="venue" value="{{ $training->venue }}">
                            </div>
                        </div>
                        <div class="form-group row mb-5">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-8">
                                <select id="participant" multiple="multiple" class="form-select form-control" name="participant[]">
                                    ...
                                    @foreach ($employee as $item)
                                        <option {{ $item->selectedPart ? 'selected' : '' }} value="{{ $item->id }}">
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
...
@push('scripts')
    ...
    <script>
        ...
        $('.timepicker').timepicker({
            ...
            minTime: '08',
            maxTime: '17:00',
            defaultTime: "{{ date('H', strtotime($training->training_date)) }}",
            startTime: '08:00',
            dynamic: false,
            dropdown: true,
            scrollbar: true
        ...
        let fileMat1 = "{{ $training->material_1 }}";
        if (fileMat1 != "") {
            pond1.addFile("{{ asset($training->material_1) }}")
        }
        const material2 = document.getElementById('material2');
        const pond2 = FilePond.create(material2);
        let fileMat2 = "{{ $training->material_2 }}";
        if (fileMat2 != "") {
            pond2.addFile("{{ asset($training->material_2) }}")
        }
        const material3 = document.getElementById('material3');
        const pond3 = FilePond.create(material3);
        let fileMat3 = "{{ $training->material_3 }}";
        if (fileMat3 != "") {
            pond3.addFile("{{ asset($training->material_3) }}")
        }

        let form = $('#formTraining');
        let btnSave = $('#btnSave');
        let tags = [];
        let selectedTag = @json($training->tags);
        if (selectedTag.length) {
            for (let z = 0; z < selectedTag.length; z++) {
                tags.push({
                    tag_id: selectedTag[z].tag_id.toString(),
                    id: selectedTag[z].id,
                    is_delete: false
                });
            }
        ...
        function getTag() {
            ...
            route = route.replace(':id', "{{ $training->id }}");
            $.ajax({
                type: "GET",
                url: route,
                dataType: 'json',
        ...
        function save() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    let route = "{{ route('trainings.edit', ':id') }}";
                    route = route.replace(':id', "{{ $training->id }}");
        ...
        function selectTag(ids, val) {
            ...
            if (status == '1') {
                ...
                if (marked == true) {
                    tags.push({tag_id: val.toString(), id: 'NULL', 'is_delete': false});
                }
            }
            console.log(tags);
            $('#tagsInput').val(JSON.stringify(tags));
